...
| *Buy* | `buy SPECIFIER AMOUNT` | Adds AMOUNT of coin to the specified account.
| *Sell* | `sell SPECIFIER AMOUNT` | Subtracts AMOUNT of coin from the specified account.
| *Tag* | `tag SPECIFIER TAG` | Tags the specified account with TAG name.
| *List* | `list` | Lists all coin accounts.
| *Find* | `find CONDITION` | Finds all coin accounts that fit the CONDITION.
| *View* | `view SPECIFIER` | Views information about the specified account.
| *Notify* | `notify {SPECIFIER,TAG} CONDITION` | Sets CoinBook to notify the user when the CONDITION has been met.
| *Sort* | `order OPTION/{a,d}...` | Sorts the coin accounts in the CoinBook based on their OPTION.
